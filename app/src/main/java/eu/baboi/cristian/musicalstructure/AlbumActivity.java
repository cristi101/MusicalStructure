...
public class AlbumActivity extends AppCompatActivity implements PagingCallbacks.Progress {
    ...
    private class AlbumCallbacks implements LoaderManager.LoaderCallbacks<Loaders.AlbumResult> {
        ...
        public void onLoadFinished(@NonNull Loader<Loaders.AlbumResult> loader, Loaders.AlbumResult data) {
            ...
            if (data == null) {
                Toast.makeText(AlbumActivity.this, "There is something wrong with your Internet connection", Toast.LENGTH_LONG).show();
                return; //no results
            }

            if (data.error != null) {
                Toast.makeText(AlbumActivity.this, String.format("Error status: %d\n%s", data.error.status, data.error.message), Toast.LENGTH_LONG).show();
                return;
            }
            if (data.aerror != null) {
                Toast.makeText(AlbumActivity.this, String.format("Authentication error: %s\n%s", data.aerror.error, data.aerror.error_description), Toast.LENGTH_LONG).show();
                return;
            }
